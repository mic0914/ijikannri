...
const isolationState = api.createPortState(); isolationState.structureId = caissonBreakwater.id;
const isolationSpan = isolationState.spans[0], isolationSource = breakwaterTargets[0];
const isolationPhoto1 = api.appendPortPhoto(isolationState, 'mobile', isolationSpan.id, isolationSource.id, '', { id: 'isolation-photo-1', data: 'data:image/jpeg;base64,/9j/2Q==', mimeType: 'image/jpeg' }).photo;
const isolationPhoto2 = api.appendPortPhoto(isolationState, 'mobile', isolationSpan.id, isolationSource.id, isolationPhoto1.id, { id: 'isolation-photo-2', data: 'data:image/jpeg;base64,/9j/2Q==', mimeType: 'image/jpeg' }).photo;
const isolationPhoto3 = api.appendPortPhoto(isolationState, 'mobile', isolationSpan.id, isolationSource.id, isolationPhoto2.id, { id: 'isolation-photo-3', data: 'data:image/jpeg;base64,/9j/2Q==', mimeType: 'image/jpeg' }).photo;
assert.notEqual(isolationPhoto1, isolationPhoto2);
assert.notEqual(isolationPhoto2, isolationPhoto3);
assert.equal(new Set([isolationPhoto1.id, isolationPhoto2.id, isolationPhoto3.id]).size, 3, 'every added photo must have an independent id');
let isolated1 = api.updatePortPhotoComponent(isolationSpan, isolationPhoto1.id, 'ケーソン', breakwaterTargets).photo;
Object.assign(isolated1, { rating: 'a', ratedAt: '2026-08-23T01:00:00.000Z', conditionComment: 'なし', conditionFreeText: '' });
let isolated2 = api.updatePortPhotoComponent(isolationSpan, isolationPhoto2.id, '上部工', breakwaterTargets).photo;
Object.assign(isolated2, { rating: 'b', ratedAt: '2026-08-23T01:01:00.000Z', conditionComment: 'なし', conditionFreeText: '' });
assert.equal(api.updatePortPhotoComponent(isolationSpan, isolationPhoto3.id, '消波工', breakwaterTargets).status, 'choice-required');
let isolated3 = api.updatePortPhotoInspectionItem(isolationSpan, isolationPhoto3.id, 'criteria_03_item_005', breakwaterTargets).photo;
Object.assign(isolated3, { rating: 'c', ratedAt: '2026-08-23T01:02:00.000Z', conditionComment: 'なし', conditionFreeText: '' });
const isolated1Snapshot = JSON.stringify(isolated1), isolated2Snapshot = JSON.stringify(isolated2);
isolated3 = api.updatePortPhotoInspectionItem(isolationSpan, isolationPhoto3.id, 'criteria_03_item_006', breakwaterTargets).photo;
assert.equal(isolated3.rating, null);
assert.equal(isolated3.conditionComment, '');
assert.equal(JSON.stringify(api.findPortPhoto(isolationSpan, isolationPhoto1.id).photo), isolated1Snapshot, 'changing photo 3 must not affect photo 1');
assert.equal(JSON.stringify(api.findPortPhoto(isolationSpan, isolationPhoto2.id).photo), isolated2Snapshot, 'changing photo 3 must not affect photo 2');
isolated3 = api.updatePortPhotoInspectionItem(isolationSpan, isolationPhoto3.id, 'criteria_03_item_005', breakwaterTargets).photo;
Object.assign(isolated3, { rating: 'c', ratedAt: '2026-08-23T01:03:00.000Z', conditionComment: 'なし', conditionFreeText: '' });
const isolated3Snapshot = JSON.stringify(isolated3);
let isolated2Change = api.updatePortPhotoComponent(isolationSpan, isolationPhoto2.id, '施設全体', breakwaterTargets);
assert.equal(isolated2Change.status, 'choice-required');
isolated2Change = api.updatePortPhotoInspectionItem(isolationSpan, isolationPhoto2.id, 'criteria_03_item_003', breakwaterTargets);
assert.equal(isolated2Change.photo.rating, null, 'only changed photo 2 assessment is cleared');
assert.equal(JSON.stringify(api.findPortPhoto(isolationSpan, isolationPhoto1.id).photo), isolated1Snapshot, 'changing photo 2 must not affect photo 1');
assert.equal(JSON.stringify(api.findPortPhoto(isolationSpan, isolationPhoto3.id).photo), isolated3Snapshot, 'changing photo 2 must not affect photo 3');
isolated2 = api.updatePortPhotoComponent(isolationSpan, isolationPhoto2.id, '上部工', breakwaterTargets).photo;
Object.assign(isolated2, { rating: 'b', ratedAt: '2026-08-23T01:04:00.000Z', conditionComment: 'なし', conditionFreeText: '' });
api.refreshPortSpanStatuses(isolationSpan, breakwaterTargets);
const isolationSummary = api.summarizePortFacility(isolationState);
assert.equal(isolationSummary.matrix.rows.find((row) => row.component === 'ケーソン').cells[0].counts.a, 1);
assert.equal(isolationSummary.matrix.rows.find((row) => row.component === '上部工').cells[0].counts.b, 1);
assert.equal(isolationSummary.matrix.rows.find((row) => row.component === '消波工').cells[0].counts.c, 1);
const isolationOutput = api.buildPortOutputData(isolationState);
const isolationEntries = isolationOutput.spans[0].entries.flatMap((entry) => entry.photos.map((result) => [result.photo.id, result.component, result.photo.rating]));
assert.deepEqual(Array.from(isolationEntries), [['isolation-photo-1', 'ケーソン', 'a'], ['isolation-photo-2', '上部工', 'b'], ['isolation-photo-3', '消波工', 'c']]);
assert.ok(api.buildPortExcelBytes(isolationOutput).length > 0);
const isolationPdf = api.buildPortPdfHtml(isolationOutput);
for (const value of ['ケーソン', '上部工', '消波工', '評価 a', '評価 b', '評価 c']) assert.match(isolationPdf, new RegExp(value));
let isolationReload = api.normalizePortState(JSON.parse(JSON.stringify(isolationState)));
assert.deepEqual(Array.from(['isolation-photo-1', 'isolation-photo-2', 'isolation-photo-3'], (id) => { const photo = api.findPortPhoto(isolationReload.spans[0], id).photo; return [photo.component, photo.rating]; }), [['ケーソン', 'a'], ['上部工', 'b'], ['消波工', 'c']]);
const reloadedIsolationSpan = isolationReload.spans[0];
const isolatedContent = (photo) => JSON.stringify({ ...photo, order: 0 });
const beforeDeleteContent2 = isolatedContent(api.findPortPhoto(reloadedIsolationSpan, isolationPhoto2.id).photo), beforeDeleteContent3 = isolatedContent(api.findPortPhoto(reloadedIsolationSpan, isolationPhoto3.id).photo);
const deleteRecord = api.findPortPhoto(reloadedIsolationSpan, isolationPhoto1.id).record;
deleteRecord.photos = deleteRecord.photos.filter((photo) => photo.id !== isolationPhoto1.id);
api.reindexPortSpanPhotos(reloadedIsolationSpan);
assert.equal(isolatedContent(api.findPortPhoto(reloadedIsolationSpan, isolationPhoto2.id).photo), beforeDeleteContent2);
assert.equal(isolatedContent(api.findPortPhoto(reloadedIsolationSpan, isolationPhoto3.id).photo), beforeDeleteContent3);
